 ...
 * #L%
 */
package org.alfresco.extension.repo.importer.view;
...
import org.alfresco.extension.repo.importer.Importer;
import org.alfresco.extension.repo.importer.Parser;
...
public class ViewParser implements Parser {
    ...
    private static final String VIEW_NAME_ATTR = "name";
    private static final String VIEW_PROPERTY_ATTR = "property";
    ...
    private static final QName VIEW_VERSIONS = QName.createQName(NamespaceService.REPOSITORY_VIEW_1_0_URI, "versions");
    private static final QName VIEW_VERSION = QName.createQName(NamespaceService.REPOSITORY_VIEW_1_0_URI, "version");
    private static final QName VIEW_VERSION_LABEL = QName.createQName(NamespaceService.REPOSITORY_VIEW_1_0_URI,
            "versionLabel");
    private static final QName VIEW_VERSION_PROPERTY = QName.createQName(NamespaceService.REPOSITORY_VIEW_1_0_URI,
            "versionProperty");
    private static final QName VIEW_VERSION_CONTENT = QName.createQName(NamespaceService.REPOSITORY_VIEW_1_0_URI,
            "versionContent");
     ...
     * (non-Javadoc)
     * 
     * @see org.alfresco.repo.importer.Parser#parse(java.io.Reader,
        ...
        try {
            ...
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Failed to import package at line " + xpp.getLineNumber() + "; column "
                            + xpp.getColumnNumber() + " due to error: ", e);
                }
    ...
    private void processStartElement(XmlPullParser xpp, ParserContext parserContext)
        ...
        if (defName.equals(VIEW_METADATA)) {
            ...
            if (logger.isDebugEnabled())
                ...
                || defName.equals(VIEW_ACL) || defName.equals(VIEW_VERSIONS)) {
            if (element instanceof NodeItemContext) {
                throw new ImporterException(
                        "Cannot nest element " + defName + " within " + ((NodeItemContext) element).getElementName());
            }
            ...
            if (element instanceof MetaDataContext) {
                ...
                if (itemName.equals(VIEW_ASPECTS)) {
                    ...
                    if (def == null) {
                        throw new ImporterException(
                                "Association name " + defName + " is not valid; cannot find in Repository dictionary");
                    }
                    processStartAssoc(xpp, (AssociationDefinition) def, parserContext);
                } else if (itemName.equals(VIEW_ACL)) {
                    processAccessControlEntry(xpp, parserContext);
                } else if (itemName.equals(VIEW_VERSIONS)) {
                    if (defName.equals(VIEW_VERSION)) {
                        processVersionElement(xpp, parserContext);
                    }
                }
            }
        }
     ...
     * @throws IOException
     */
    private void processVersionElement(XmlPullParser xpp, ParserContext parserContext)
            throws XmlPullParserException, IOException {
        NodeContext node = peekNodeContext(parserContext.elementStack);

        QName defName = getName(xpp);
        if (!defName.equals(VIEW_VERSION)) {
            throw new ImporterException("Expected start element " + VIEW_VERSION);
        }

        // Parse version label from child element
        String versionLabel = null;
        Map<String, String> parsedVersionProperties = new HashMap<String, String>();
        Map<String, String> parsedVersionContentByProperty = new HashMap<String, String>();
        int eventType = xpp.next();
        while (eventType != XmlPullParser.END_TAG) {
            if (eventType == XmlPullParser.START_TAG) {
                defName = getName(xpp);
                if (defName.equals(VIEW_VERSION_LABEL)) {
                    versionLabel = readRequiredSimpleElementText(xpp, defName);
                } else if (defName.equals(VIEW_VERSION_PROPERTY)) {
                    String propertyName = xpp.getAttributeValue(NamespaceService.REPOSITORY_VIEW_1_0_URI,
                            VIEW_NAME_ATTR);
                    String propertyValue = readOptionalSimpleElementText(xpp, defName);
                    if (propertyName != null && propertyName.length() > 0) {
                        parsedVersionProperties.put(propertyName, propertyValue);
                    }
                } else if (defName.equals(VIEW_VERSION_CONTENT)) {
                    String propertyQName = xpp.getAttributeValue(NamespaceService.REPOSITORY_VIEW_1_0_URI,
                            VIEW_PROPERTY_ATTR);
                    String contentDescriptor = readOptionalSimpleElementText(xpp, defName);
                    if (propertyQName != null && propertyQName.length() > 0) {
                        parsedVersionContentByProperty.put(propertyQName, contentDescriptor);
                    }
                } else {
                    // Skip unknown version elements for future extensibility
                    if (logger.isDebugEnabled())
                        logger.debug("Skipping unknown version element: " + defName);
                }
        ...
        // Add version label to node context if present
        if (versionLabel != null && !versionLabel.isEmpty()) {
            node.addVersionHistoryEntry(new NodeContext.VersionImportData(versionLabel, parsedVersionProperties,
                    parsedVersionContentByProperty));
            if (logger.isDebugEnabled())
                logger.debug("Added version label: " + versionLabel);
        }

        // extract end of version
        defName = getName(xpp);
        if (!defName.equals(VIEW_VERSION)) {
            throw new ImporterException("Expected end element " + VIEW_VERSION);
        }
    }

    /**
     * Read a simple element that must contain a value, allowing either text content
     * or a self-closing tag.
     *
     * @param xpp         XmlPullParser
     * @param elementName element name currently open
     * @return parsed text value
     * @throws XmlPullParserException if the element cannot be parsed
     * @throws IOException            on parser I/O errors
     */
    private String readRequiredSimpleElementText(XmlPullParser xpp, QName elementName)
            throws XmlPullParserException, IOException {
        String value = readOptionalSimpleElementText(xpp, elementName);
        if (value == null || value.length() == 0) {
            throw new ImporterException("Element " + elementName + " must have a value");
        }
        return value;
    }

    /**
     * Read a simple element that may be self-closing or contain text.
     *
     * @param xpp         XmlPullParser
     * @param elementName element name currently open
     * @return parsed text value, or an empty string for self-closing elements
     * @throws XmlPullParserException if the element cannot be parsed
     * @throws IOException            on parser I/O errors
     */
    private String readOptionalSimpleElementText(XmlPullParser xpp, QName elementName)
            throws XmlPullParserException, IOException {
        int eventType = xpp.next();
        String value = "";
        if (eventType == XmlPullParser.TEXT) {
        ...
        if (eventType != XmlPullParser.END_TAG) {
            throw new ImporterException("Expected end element " + elementName);
        }

        QName endDefName = getName(xpp);
        if (!elementName.equals(endDefName)) {
            throw new ImporterException("Expected end element " + elementName);
        }
        return value;
    }
        ...
        if (eventType == XmlPullParser.END_TAG) {
            ...
            while (eventType == XmlPullParser.START_TAG) {
                ...
                if (eventType == XmlPullParser.END_TAG) {
                    ...
                    throw new ImporterException(
                            "Value for property " + propertyName + " has not been defined correctly - missing end tag");
                }
                eventType = xpp.next();
                if (eventType == XmlPullParser.TEXT) {
                    eventType = xpp.next();
    ...
    private void processEndType(ParserContext parserContext, NodeContext node) {
        ...
        // Replay historical versions after the live node exists and base properties are in place.
        node.getImporter().applyVersionHistory(node, nodeRef);
    ...
    @Override
    public void parse(Reader viewReader, org.alfresco.repo.importer.Importer importer) {
        if (importer instanceof Importer) {
            parse(viewReader, (Importer) importer);
            return;
        }
        throw new ImporterException("Unsupported importer implementation: " + importer.getClass().getName());
    }
