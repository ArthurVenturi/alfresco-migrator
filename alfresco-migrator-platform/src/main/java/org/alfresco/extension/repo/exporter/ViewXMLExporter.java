 ...
 * #L%
 */
package org.alfresco.extension.repo.exporter;

import java.io.InputStream;
import java.util.List;
import java.util.Locale;

import org.alfresco.extension.service.cmr.view.Exporter;
import org.alfresco.extension.service.cmr.view.ExporterContext;
import org.alfresco.extension.service.cmr.view.ExporterException;
    ...
    implements Exporter
{
    ...
    private static final String VERSIONS_LOCALNAME = "versions";
    private static final String VERSION_LOCALNAME = "version";
    private static final String VERSIONLABEL_LOCALNAME = "versionLabel";
    private static final String VERSIONPROPERTY_LOCALNAME = "versionProperty";
    private static final String VERSIONCONTENT_LOCALNAME = "versionContent";
    private static final String NAME_LOCALNAME = "name";
    private static final String PROPERTY_LOCALNAME = "property";
    ...
    private static QName VERSIONS_QNAME;
    private static QName VERSION_QNAME;
    private static QName VERSIONLABEL_QNAME;
    private static QName VERSIONPROPERTY_QNAME;
    private static QName VERSIONCONTENT_QNAME;
    ...
    ViewXMLExporter(NamespaceService namespaceService, NodeService nodeService, SearchService searchService,
            DictionaryService dictionaryService, PermissionService permissionService,
            VersionService versionService, ContentHandler contentHandler)
    {
        ...
        VERSIONS_QNAME = QName.createQName(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONS_LOCALNAME, namespaceService);
        VERSION_QNAME = QName.createQName(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSION_LOCALNAME, namespaceService);
        VERSIONLABEL_QNAME = QName.createQName(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONLABEL_LOCALNAME, namespaceService);
        VERSIONPROPERTY_QNAME = QName.createQName(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONPROPERTY_LOCALNAME, namespaceService);
        VERSIONCONTENT_QNAME = QName.createQName(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONCONTENT_LOCALNAME, namespaceService);
        NAME_QNAME = QName.createQName(NamespaceService.REPOSITORY_VIEW_PREFIX, NAME_LOCALNAME, namespaceService);
        PROPERTY_QNAME = QName.createQName(NamespaceService.REPOSITORY_VIEW_PREFIX, PROPERTY_LOCALNAME, namespaceService);
        ...
        try
        {
            ...
            if (value instanceof NodeRef && referenceType.equals(ReferenceType.PATHREF))
            {
                ...
                if (nodeRef.getStoreRef().equals(valueNodeRef.getStoreRef()))
                {
                   ...
                   // NOTE: makes the exporter treat custom/tag-like category properties the same way as cm:categories
                    boolean categoryTypedProperty = dataTypeDef != null && DataTypeDefinition.CATEGORY.equals(dataTypeDef.getName());
                    if (property.equals(ContentModel.PROP_CATEGORIES) || categoryTypedProperty)
                    {
                        // Category references (including taggable properties) may point outside the export tree.
                        // Use full paths so import can resolve references consistently.
    ...
    /**
     * Start export of version history
     * 
    ...
    public void startVersions(NodeRef nodeRef)
    {
        try
        {
            contentHandler.startElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONS_LOCALNAME, VERSIONS_QNAME.toPrefixString(), EMPTY_ATTRIBUTES);
        }
        catch (SAXException e)
        {
            throw new ExporterException("Failed to start versions element", e);
        }
    }
    
    /**
     * Start export of a version
     * 
     * @param nodeRef  the node reference
     * @param versionLabel  the version label
     */
    public void startVersion(NodeRef nodeRef, String versionLabel)
    {
        try
        {
            contentHandler.startElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSION_LOCALNAME, VERSION_QNAME.toPrefixString(), EMPTY_ATTRIBUTES);
            contentHandler.startElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONLABEL_LOCALNAME, VERSIONLABEL_QNAME.toPrefixString(), EMPTY_ATTRIBUTES);
            contentHandler.characters(versionLabel.toCharArray(), 0, versionLabel.length());
            contentHandler.endElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONLABEL_LOCALNAME, VERSIONLABEL_QNAME.toPrefixString());
        }
        catch (SAXException e)
        {
            throw new ExporterException("Failed to start version element", e);
        }
    }

    /**
     * Export a version-scoped property
     * 
     * @param nodeRef  the node reference
     * @param propertyName  version property name
     * @param propertyValue  version property value
     */
    public void versionProperty(NodeRef nodeRef, String propertyName, String propertyValue)
    {
        try
        {
            AttributesImpl attrs = new AttributesImpl();
            attrs.addAttribute(NamespaceService.REPOSITORY_VIEW_1_0_URI, NAME_LOCALNAME, NAME_QNAME.toPrefixString(), null, propertyName);
            contentHandler.startElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONPROPERTY_LOCALNAME, VERSIONPROPERTY_QNAME.toPrefixString(), attrs);
            if (propertyValue != null)
            {
                contentHandler.characters(propertyValue.toCharArray(), 0, propertyValue.length());
            }
            contentHandler.endElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONPROPERTY_LOCALNAME, VERSIONPROPERTY_QNAME.toPrefixString());
        }
        catch (SAXException e)
        {
            throw new ExporterException("Failed to export version property", e);
        }
    }

    /**
     * Export a version content reference
     * 
     * @param nodeRef  the node reference
     * @param propertyQName  frozen property qname
     * @param content  content stream (already handled by URL exporter)
     * @param contentData  content descriptor
     */
    public void versionContent(NodeRef nodeRef, String propertyQName, InputStream content, ContentData contentData)
    {
        try
        {
            AttributesImpl attrs = new AttributesImpl();
            attrs.addAttribute(NamespaceService.REPOSITORY_VIEW_1_0_URI, PROPERTY_LOCALNAME, PROPERTY_QNAME.toPrefixString(), null, propertyQName);
            contentHandler.startElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONCONTENT_LOCALNAME, VERSIONCONTENT_QNAME.toPrefixString(), attrs);
            if (contentData != null)
            {
                String value = DefaultTypeConverter.INSTANCE.convert(String.class, contentData);
                contentHandler.characters(value.toCharArray(), 0, value.length());
            }
            contentHandler.endElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONCONTENT_LOCALNAME, VERSIONCONTENT_QNAME.toPrefixString());
        }
        catch (SAXException e)
        {
            throw new ExporterException("Failed to export version content reference", e);
        }
    }
    
    /**
     * End export of a version
     * 
     * @param nodeRef  the node reference
     * @param versionLabel  the version label
     */
    public void endVersion(NodeRef nodeRef, String versionLabel)
    {
        try
        {
            contentHandler.endElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSION_LOCALNAME, VERSION_QNAME.toPrefixString());
        }
        catch (SAXException e)
        {
            throw new ExporterException("Failed to end version element", e);
        }
    }
    
    /**
     * End export of version history
     * 
     * @param nodeRef  the node reference
     */
    public void endVersions(NodeRef nodeRef)
    {
        try
        {
            contentHandler.endElement(NamespaceService.REPOSITORY_VIEW_PREFIX, VERSIONS_LOCALNAME, VERSIONS_QNAME.toPrefixString());
        }
        catch (SAXException e)
        {
            throw new ExporterException("Failed to end versions element", e);
        }
    }
}
