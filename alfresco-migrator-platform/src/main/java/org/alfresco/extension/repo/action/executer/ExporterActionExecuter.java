...
import org.alfresco.extension.repo.exporter.ACPExportPackageHandler;
...
public class ExporterActionExecuter extends ActionExecuterAbstractBase {
    public static final String NAME = "migratorExporter";
    ...
    // Aligned with importer action to keep one user-facing switch for history transport.
    ...
    public void executeImpl(Action ruleAction, NodeRef actionedUponNodeRef) {
        ...
        try {
            ...
            Boolean withVersionHistory = (Boolean) ruleAction.getParameterValue(PARAM_INCLUDE_VERSION_HISTORY);
            if (withVersionHistory != null) {
                includeVersionHistory = withVersionHistory.booleanValue();
            }

            // When enabled, ExporterComponent emits <view:versions> with metadata and content references.
            params.setExportVersionHistory(includeVersionHistory);
    ...
    protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
        ...
        paramList.add(new ParameterDefinitionImpl(PARAM_INCLUDE_VERSION_HISTORY, DataTypeDefinition.BOOLEAN, false,
                getParamDisplayLabel(PARAM_INCLUDE_VERSION_HISTORY)));
    }
    ...
    private NodeRef createExportZip(Action ruleAction, NodeRef actionedUponNodeRef) {
        ...
        // add the default Alfresco content package extension if an extension hasn't
        // been given
        if (!packageName.endsWith("." + ACPExportPackageHandler.ACP_EXTENSION)) {
            packageName += (packageName.charAt(packageName.length() - 1) == '.') ? ACPExportPackageHandler.ACP_EXTENSION
        ...
        // build a description string to be set on the node representing the content
        // package
        String desc = "";
