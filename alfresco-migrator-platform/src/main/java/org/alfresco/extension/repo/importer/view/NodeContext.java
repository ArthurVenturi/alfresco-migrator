...
import org.alfresco.extension.repo.importer.ImportVersion;
import org.alfresco.extension.repo.importer.ImportNode;
...
public class NodeContext extends ElementContext
    ...
    private List<VersionImportData> versionHistoryEntries = new ArrayList<VersionImportData>();
    ...
    public void addPropertyCollection(QName property) {
        // Do not import properties of sys:referenceable or cm:versionable or
        // cm:copiedfrom
     ...
     * (non-Javadoc)
     * 
     * @see org.alfresco.repo.importer.ImportNode#getPropertyDataType(org.alfresco.
    ...
    public void addAccessControlEntry(AccessStatus accessStatus, String authority, String permission) {
        // Note: Map guest permission to Consumer permission - this is to handle the
        // case where
    ...
    public String toString() {
        return "NodeContext[childName=" + getChildName() + ",type=" + (typeDef == null ? "null" : typeDef.getName())
                + ",nodeRef=" + nodeRef +
    ...
    /**
     * Get all version labels for this node
     * 
     * @return list of version labels
     */
    public List<String> getVersionLabels() {
        List<String> versionLabels = new ArrayList<String>();
        for (VersionImportData versionData : versionHistoryEntries) {
            if (versionData.getVersionLabel() != null && !versionData.getVersionLabel().isEmpty()) {
                versionLabels.add(versionData.getVersionLabel());
            }
        }
        return versionLabels;
    }

    public List<ImportVersion> getVersions() {
        List<ImportVersion> versions = new ArrayList<ImportVersion>();
        for (VersionImportData versionData : versionHistoryEntries) {
            if (versionData == null) {
                continue;
            }

            final String label = versionData.getVersionLabel();
            final String description = versionData.getVersionProperties() == null ? null
                    : versionData.getVersionProperties().get("description");
            final String author = versionData.getVersionProperties() == null ? null
                    : versionData.getVersionProperties().get("creator");

            versions.add(new ImportVersion() {
    ...
    /**
     * Add a version history entry with metadata and content references.
     * 
     * @param versionData parsed version data
     */
    public void addVersionHistoryEntry(VersionImportData versionData) {
        if (versionData != null) {
            versionHistoryEntries.add(versionData);
        }
    }

    /**
     * @return parsed version history entries
     */
    public List<VersionImportData> getVersionHistoryEntries() {
        return versionHistoryEntries;
    }

    /**
     * Holds parsed version history data from import XML.
     */
    public static class VersionImportData {
        private final String versionLabel;
        private final Map<String, String> versionProperties;
        private final Map<String, String> versionContentByPropertyQName;

        public VersionImportData(String versionLabel, Map<String, String> versionProperties,
                Map<String, String> versionContentByPropertyQName) {
            this.versionLabel = versionLabel;
            this.versionProperties = versionProperties;
            this.versionContentByPropertyQName = versionContentByPropertyQName;
        }
        ...
        public Map<String, String> getVersionContentByPropertyQName() {
            return versionContentByPropertyQName;
        }
    }
