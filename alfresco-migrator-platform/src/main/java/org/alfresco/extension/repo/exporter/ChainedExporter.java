    ...
    implements Exporter
{
    ...
    {
        // Null exporters are ignored so the chain can safely compose optional callbacks.
    ...
    {
        // Version/history callbacks are broadcast in the same order as provided at construction.
        for (Exporter exporter : exporters)
        {
    ...
    {
        // Keep raw stream/content metadata untouched; downstream exporters decide how to persist it.
        for (Exporter exporter : exporters)
        {
            exporter.versionContent(nodeRef, propertyQName, content, contentData);
        }
    }
